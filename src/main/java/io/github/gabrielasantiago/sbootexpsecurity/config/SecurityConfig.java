...
@Configuration
@EnableWebSecurity
@EnableMethodSecurity(securedEnabled = true) //habilita a configuração das permissões e autorização no controller
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, 
		...
		return http
				.csrf(AbstractHttpConfigurer::disable) //protege a aplicação através de um token 
				.authorizeHttpRequests(customizer -> {
			customizer.requestMatchers("/public").permitAll();
			//customizer.requestMatchers("/admin").hasRole("ADMIN"); comentado para definir as persmissoes via controllers
